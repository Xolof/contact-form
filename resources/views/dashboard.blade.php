                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @foreach($messages as $message)
                        <div class="py-4">
                            ...
                            @endphp
                            <form method="POST" action="{{ route('update-message') }}" class="mb-2">
                            @csrf
                                ...
                                <input type="submit" id="save" name="save"
                                    ...
                                    class="cursor-pointer justify-center max-w-lg inline-flex items-center px-4 py-2 bg-blue-500 dark:bg-blue-400 border border-transparent rounded-md font-semibold text-xs text-white dark:text-blue-900 uppercase tracking-widest hover:bg-blue-700 dark:hover:bg-white focus:bg-blue-700 dark:focus:bg-white active:bg-blue-900 dark:active:bg-blue-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-blue-800 transition ease-in-out duration-150"/>
                            </form>
                            <form method="POST" action="{{ route('delete-message') }}">
                            @csrf
                                <input type="hidden" id="messageId" name="messageId" value="{{ $message["id"] }}"/>
                                <input type="submit" id="delete" name="delete" value="Delete" class="cursor-pointer justify-center max-w-lg inline-flex items-center px-4 py-2 bg-red-500 dark:bg-red-400 border border-transparent rounded-md font-semibold text-xs text-white dark:text-red-900 uppercase tracking-widest hover:bg-red-700 dark:hover:bg-white focus:bg-red-700 dark:focus:bg-white active:bg-red-900 dark:active:bg-red-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-red-800 transition ease-in-out duration-150"/>
                            </form>
                        </div>
                    @endforeach
                </div>
